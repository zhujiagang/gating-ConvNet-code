...
bool ReadSegmentFLOW_RGBToDatum(const string& filename, const int label,
    const vector<int> offsets, const int height, const int width, const int length, Datum* datum, bool is_flow){
  ...
  for (int i = 0; i < offsets.size(); ++i){
    ...
    for (int file_id = 1; file_id < length+1; ++file_id){
      sprintf(tmp, "flow_%c_%05d.jpg", 'x', int(file_id+offset));
      string filename_x = filename + "/" + tmp;
      cv::Mat cv_img_origin_x = cv::imread(filename_x, CV_LOAD_IMAGE_GRAYSCALE);
      sprintf(tmp, "flow_%c_%05d.jpg", 'y', int(file_id+offset));
      ...
      if (!cv_img_origin_x.data){
        ...
        LOG(INFO) << "Could not load file " << filename_x << " or " << filename_y<< "i:"<<i <<"file_id"<<file_id;
        LOG(INFO) << "!!!!x!!!!!";
        return false;
      }
      ...
      if (file_id==1 && i==0){
          if (is_flow){
                ...
                int num_channels = 5;
                datum->set_channels(2*length*offsets.size() + 3*1*offsets.size() );
          }
...
//if(is_flow == false){
    ...
    int offset = offsets[i];
    for (int file_id = 1; file_id < length+1; ++file_id){
      sprintf(tmp, "img_%05d.jpg", int(file_id+offset));
      string filename_t = filename + "/" + tmp;
      cv::Mat cv_img_origin = cv::imread(filename_t, CV_LOAD_IMAGE_COLOR);

      if (!cv_img_origin.data){
        LOG(INFO) << "!!!!!length!!!!!"<< length;
        LOG(INFO) << "Could not load file " << filename_t<< "i:"<<i <<"file_id"<<file_id;
        LOG(INFO) << "!!!!rgb!!!!!";
        return false;
      }
